...
/////////////////////////////////////////////////////////////////
	...
	/* Time Interval for get time NTPClient */
	  ...
	  const long intervalLoop = 1000;  //1 second
...
void queryData()
{
	...
	if(Pump == 0 && alarmPump == 0){
		...
		if(startHour == currentTime.Hours){
			...
			//Serial.print(abs(startMinute - currentTime.Minutes));
			if(abs(startMinute - currentTime.Minutes)<10){
				setPowerPump(pumpOn);
				statusPump = 1;
				glResetTimerFlag = 0;
		...
		else if(abs(startHour - currentTime.Hours)>=2){
			statusPump = 0;
			glResetTimerFlag = 1;
			//Serial.print(2);
		}
